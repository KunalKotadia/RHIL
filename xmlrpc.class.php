...
class TestlinkXMLRPCServer extends IXR_Server
{
  ...
  public static $countReqParamName = "countreq";
  ...
  public static $documentIDParamName = "documentid";
  ...
  public static $typeParamName = "type";
  ...
  {    
    ...
    $this->tree_mgr = new tree($this->dbObj);
  ...
  */
  ...
  public function createRequirementSpecification($args)
  {
    ...
    switch($action)
    {
      case 'update':
        $operation='updateTestSuite';
        $opt = array(self::$orderParamName => 0,
           self::$actionOnDuplicatedNameParamName => 'block');
      break;
      ...
      $operation=__FUNCTION__;
      $opt = array(self::$actionOnDuplicatedNameParamName => 'block');
    ...
    if($status_ok)
    {
      ...
      if( $this->_isParamPresent(self::$parentIDParamName) )
      {
        ...
        // if parentid exists it must:
        // be a test suite id 
        $node_info = $reqSpecMgr->get_by_id($args[self::$parentIDParamName]);
        if( !($status_ok=!is_null($node_info)) )
        {
          $msg=sprintf(INVALID_PARENT_TESTSUITEID_STR,
          $args[self::$parentIDParamName],$args[self::$testSuiteNameParamName]);
    ...
    $node_order = $this->tree_mgr->getBottomOrder($args[self::$testProjectIDParamName],array('node_type' => 'requirement_spec')) + 1;
    ...
    if($status_ok)
    {
      ...
      {
        case 'update':
          $op=$reqSpecMgr->update($args[self::$testSuiteIDParamName],
                                 $args[self::$testSuiteNameParamName],
                                 $args[self::$detailsParamName],
                                 $parent_id,
                                 $opt[self::$orderParamName]);
        ...
        case 'create':
        default:
          $option = array('actionOnDuplicate' => $opt[self::$actionOnDuplicatedNameParamName]);
          $op=$this->reqSpecMgr->create($args[self::$testProjectIDParamName], $parent_id, $args[self::$documentIDParamName],
                                 $args[self::$titleParamName], $args[self::$scopeParamName], $args[self::$countReqParamName], $args[self::$userIDParamName], $args[self::$typeParamName], 
                                 $node_order, $option);
        break;  
      }
